...
│   ├── download_pubmed.sh
│   ├── parse_xml.R
│   ├── tidy_text_processing.R
│   ├── lda_topic_model.R
│   ├── word_trends_forecast.R
│
│
├── data/
│   ├── raw/                  # raw XML files downloaded here
│   ├── clean/                # Intermediate files such as token tables, matrices│
│
├── figures/                  # PNG visualisations generated by R scripts
│
├── Snakefile                 # The core workflow definition
├── config.yaml               # Snakemake profile for BluePebble (cluster settings)
├── workflow_config.yaml      # Pipeline parameters (search terms, test mode, etc.)
├── env_2361392.yml           # Cross-platform Conda environment 
...
-word_trends.png — trends in key terms across years

-word_trends_forecast.png — predicted trends
