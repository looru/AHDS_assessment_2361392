...
- An empty directory `raw/`, which is used by the pipeline as the location for downloaded raw PubMed XML files.
- An empty directory `clean/`, which is used as the output location for cleaned data files generated by the R and Snakemake scripts.
- All code needed to run the four analysis steps:
  - `code/download_raw.sh` – downloads raw XML from PubMed into `raw/`.
  - `code/extract_articles.R` – parses the XML and writes `pmid_year_title_abstract.tsv` into `clean/`.
